...
const { inferOnlineFromReportTime } = require('./OnlineStatus');
...
class XSenseDeviceBase extends Homey.Device {
  ...
  async onInit() {
    ...
    // All supported models report a numeric battery level. Remove the legacy
    // low-battery alarm to avoid duplicate battery UI components in Homey.
    if (this.hasCapability('alarm_battery') && this.hasCapability('measure_battery')) {
      await this.removeCapability('alarm_battery').catch(error => {
        this.error('Failed to remove legacy battery alarm capability:', error);
      });
    }
  ...
  async _handleDeviceUpdate(deviceData) {
    ...
    try {
      ...
      let isOnline = this._normalizeBool(onlineValue);
      const inferredOnline = inferOnlineFromReportTime(deviceData);
      if (inferredOnline === true || isOnline === undefined) {
        isOnline = inferredOnline;
      }
