...
class XSenseAPI {
    ...
    for (const house of houses) {
      ...
      for (const station of stations) {
        ...
        for (const device of devices) {
          ...
          const resolvedId = device.deviceId || device.deviceSn;
          if (!resolvedId) {
  ...
  async _connectMQTTInternal(houseId, stationId) {
    ...
    client.on('reconnect', () => {
      ...
      // After reconnect, all subscriptions are lost on the broker side.
      // Clear the tracking set and re-subscribe to restore all topics.
      const prevTopics = Array.from(info.subscriptions);
      info.subscriptions.clear();
      if (prevTopics.length > 0) {
        this.debug.log(`[XSenseAPI] Reconnected — restoring ${prevTopics.length} subscriptions`);
        this._subscribeTopic(info, prevTopics);
      }
    });
  ...
  _subscribeTopic(info, topic) {
    ...
    let newTopics = topics.filter((t) => t && !info.subscriptions.has(t));
    ...
    const available = Math.max(0, maxSubs - info.subscriptions.size);
    if (available <= 0) {
    ...
    for (let i = 0; i < newTopics.length; i += maxPerSub) {
      ...
      batch.forEach((t) => info.subscriptions.add(t));
      ...
      info.client.subscribe(batch, { qos: 0 }, (err) => {
        if (err) {
          ...
          batch.forEach((t) => info.subscriptions.delete(t));
